...
def compute_joint_distribution(distribution1, distribution2):
    ...
    joint_distributions = [
        np.outer(distribution1[i], distribution2[i]).flatten() for i in range(n_rows)]
...
def perform_clustering(distributions, threshold, directory, cluster_folder):
    ...
    if os.path.exists(file_path):
        print(
            f"Loading existing cluster file for {directory} with threshold {threshold}")
...
def cluster(directorys, thress, cdf_folder, cluster_folder, return_labels=False):
    for directory in directorys:
        for thres in thress:
            ...
            labels_df = perform_clustering(
                distributions, thres, directory, cluster_folder)
            if return_labels:
                return labels_df
...
def cluster4join(directorys1, directorys2, thress, cdf_folder, cluster_folder, return_labels=False):
    for directory1, directory2 in zip(directorys1, directorys2):
        for thres in thress:
            ...
            joint_distributions = compute_joint_distribution(
                distribution1, distribution2)
            perform_clustering(joint_distributions, thres,
                               f'{directory1}-{directory2}', cluster_folder)
            if return_labels:
                return labels_df
