...
def progressive_sampling(reliability, bool_non_cand, not_sampled, entropy_array, starts, ends, sampled, frame_reward, uncertainties, o_b, o_b_s):
    if o_b < o_b_s:
        ...
        if np.random.rand() < reliability:
            ...
            selected_result = select_indices_importance(true_indices, entropy_array[true_indices], 1)
            if selected_result is None:
                return None
            return selected_result[0]

def compute_ucb(starts, ends, sampled, frame_reward, uncertainties, o_b):
    all_sampled_mask = np.array([np.all(sampled[start:end + 1]) for start, end in zip(starts, ends)])
    ...
    for start, end in zip(active_starts, active_ends):
        ...
        reward = np.mean(non_zero_rewards) if ni > 0 else np.max(uncertainties[start:end + 1])
...
def label_propagation(PD, score, sampled, cluster_boundaries, sampled_boundaries, max_i, tau, oracle, probabilities, clusters, frame_reward, uncertainties,lp_enabled):
    ...
    update_progressive_sampling_info(probabilities, clusters, frame_reward, uncertainties, left, right, max_i, score)  
...
def calculate_total_entropy(i, c_i,  probabilities, tau, score ,cluster_boundaries,leftT, rightT,entropy_array):
    ...
    if score[rightT]==1 or (rightT-right)>tau :
        rightT=right
    entropy_sum = probabilities[c_i] * np.sum(entropy_array[left:right+1]) +(1-probabilities[c_i]) * np.sum(entropy_array[leftT+1:rightT])
    return entropy_sum
...
def update_uncertainties(range_start, range_end, uncertainties, clusters, probabilities, tau, score, boundaries, reference_start, reference_end, entropy_array):
    for index in range(range_start, range_end):
        uncertainties[index] = calculate_total_entropy(index, clusters[index], probabilities, tau, score, boundaries, reference_start, reference_end, entropy_array)
...
def calculate_j_indices(i_values, a, b, IOUThreshold, total_boundaries):
    jl = np.where(i_values <= a, np.floor(IOUThreshold * (b - i_values + 1) + a - 1), np.floor(IOUThreshold * (b - a + 1) + i_values - 1)).astype(int)
    jr = np.where(i_values <= a, np.ceil((b - a + 1) / IOUThreshold + i_values - 1), np.ceil((b - i_values + 1) / IOUThreshold + a - 1)).astype(int)
...
def calculate_confidence(cand_clips, IOUThreshold, probabilities, cluster, boundaries):
    ...
    for clip in cand_clips:
        ...
        i_values, cluster_range = calculate_indices(start, end, boundaries, cluster)
        j_left, j_right = calculate_j_indices(i_values, clip[0], clip[1], IOUThreshold, total_boundaries)
        combined_prob = 0
        for i, clust_val in enumerate(cluster_range):
            cluster_j_range = np.arange(cluster[j_left[i]] + 1, cluster[j_right[i]])
            if cluster_j_range.size == 0:
                continue
            prob_matrix = probabilities[clust_val:cluster_j_range[-1] + 1]
            prob_products = np.cumprod(prob_matrix, axis=0)[cluster_j_range - clust_val]
            left_prob = 1 - probabilities[clust_val - 1]
            cluster_j_range_plus = cluster_j_range + 1
            right_prob = 1 - probabilities[np.clip(cluster_j_range_plus, 0, len(probabilities) - 1)]
